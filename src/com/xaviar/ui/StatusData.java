...
public class StatusData {
	...
	public static final String GET_ALL_ORDER_BY = DbHelper.C_CREATED_AT + " DESC";
	private static final String[] MAX_CREATED_AT_COLUMNS = { "max("+ DbHelper.C_CREATED_AT + ")" };
	...
	public long insertUpdate(ContentValues values) {
		SQLiteDatabase db = dbHelper.getWritableDatabase();
		try {
			return db.insertWithOnConflict(DbHelper.TABLE, null, values,
	...
	public int updateStatuses(ContentValues values, String selection, String[] selectionArgs) {
		SQLiteDatabase db = dbHelper.getWritableDatabase();
		try {
			int numAffecteRows =  db.update(DbHelper.TABLE, values, selection, selectionArgs);
			Log.i(TAG, "numAffectedRows=" + numAffecteRows);
			return numAffecteRows;
		} catch (Exception e) {
			Log.e(TAG, "Fail to update row,got an exception:" + e.getLocalizedMessage());
			e.printStackTrace();
			return -1;
		} finally {
			db.close();
