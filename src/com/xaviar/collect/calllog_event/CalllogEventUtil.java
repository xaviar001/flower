...
import com.xaviar.events.CalllogEventReceiver;
...
public class CalllogEventUtil {
	
	private static final String TAG = CalllogEventUtil.class.getSimpleName();
	

	public static String getAllPhoneGeneric(Context ctx) {
		 String  genericData = DbUtil.getAllGeneric(ctx,DataHolder.CALL_LOG_EVENT);
	...
	private static String generateEmptyPhoneEvent() {
		CallLogEntry phone = new CallLogEntry();
		 ...
		 allPhones.add(phone);
		 String ret = CalllogEventReceiver.toJsonArray(allPhones);
	...
	public static void save(Context ctx, CallLogEntry callLogEntry) {
		...
		userData.setSource(DataHolder.CALL_LOG_EVENT);
